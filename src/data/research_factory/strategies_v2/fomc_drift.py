"""Pre-FOMC announcement drift (Lucca-Moench), hand-written 2026-06-12.
Long the index only in the ~2 trading days into each SCHEDULED FOMC announcement,
exit at the announcement day's close. The calendar is public months in advance,
so using it is information-legal. Dates sourced from federalreserve.gov
(207 scheduled announcements 2000-2026; unscheduled meetings excluded)."""
...
PARAMS = {"entry_days_before": 2, "size_frac": 0.95}
...
class FomcDrift(Strategy):
    ...
    def init(self):
        self._anns = pd.to_datetime(ANNOUNCEMENTS)
        self._target = None

    def next(self):
        d = self.data.index[-1]
        if self.position:
            if self._target is not None and d >= self._target:
                self.position.close()
                self._target = None
            return
        future = self._anns[self._anns > d]
        if len(future) == 0:
            return
        nxt = future[0]
        if 0 < (nxt - d).days <= self.entry_days_before + 1:
            self.buy(size=self.size_frac)
            self._target = nxt


STRATEGY_CLASS = FomcDrift


def generate_signal(df):
    anns = pd.to_datetime(ANNOUNCEMENTS)
    d = df.index[-1] if isinstance(df.index, pd.DatetimeIndex) else pd.Timestamp(df["datetime"].iloc[-1])
    future = anns[anns > d]
    if len(future) and 0 < (future[0] - d).days <= PARAMS["entry_days_before"] + 1:
        return {"action": "BUY", "confidence": 60,
                "reasoning": f"pre-FOMC window: announcement {future[0].date()}"}
    return {"action": "NOTHING", "confidence": 0, "reasoning": "outside pre-FOMC window"}
