#!/bin/sh
# Driver: run the 9 PDF strategies across 5 asset classes sequentially.
# Two passes per class - processed ideas skip instantly, so pass 2 only mops up
# transient failures. Emits CYCLE ... DONE lines for monitoring.
set -u
cd /Users/sam/ai-agents
PY=.venv/bin/python
IDEAS=src/data/research_factory/ideas_pdf9.txt
